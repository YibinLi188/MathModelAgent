...
对 `question_output_coverage` 做逐项核对：题面点名的对象、时刻、参数端点、场景组合、表格行列和官方附件位置必须全部有正式输出。只给代表性样例、只展示图而缺题目要求的数表、或把辅助 CSV 当成已验收附件时判定 `FAIL`。

对连续域检查 `domain_audit`。粗时间步长、单次符号扫描或只优化粗网格最高点不能单独证明首次事件和全局包络；须有局部求根/优化，并用网格加密、误差界或独立全域复算防漏检。全区间极值遗漏端点，或未复核多个分离候选峰却写“全程最大”，判定 `FAIL`。

对优化结论检查 `claim_domain`。固定端点、固定轨迹、固定拓扑或离散候选族的结果必须在摘要和结论保留相同限定；把“固定端点下长度不随比例变化”扩写为“路径无法再缩短”，或把候选族内最优写成全局最优，判定 `FAIL`。
...
- 除官方模板明确要求外，标题和各级标题是否保持黑色；是否出现彩色标题条、装饰封面、生成/验收时间、Agent/模型版本、自动生成声明等非比赛文本。
- 摘要是否只写任务、方法、经验证结果与适用边界，是否混入自我评价、写作过程、官方答案模板说明或“结果另行保存”等元话语。
- 是否出现重复“参考文献”标题、参考文献前无理由强制分页、连续两页明显稀疏正文、整页只有小图/小表或表格被缩到难以阅读。首摘要页和自然结束的尾页可合理留白，但须在验收报告说明。
- 彩色图是否在灰度与缩印条件下仍能靠线型、点型、纹理或标签区分，关键事件/阈值是否有直接标注。
