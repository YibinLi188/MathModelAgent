...
- 以所选竞赛官方模板和同题优秀论文的共同风格为准。除官方模板明确要求外，论文标题、摘要标题和各级标题保持黑色；不用彩色标题条、渐变、阴影、装饰封面或网页式信息卡。
- 删除所有生成过程元信息，包括“生成/验收时间”、Agent 或模型版本、自动生成声明、内部测试说明和结果文件路径。比赛要求之外的说明不得出现在标题页、页眉页脚、摘要或正文。
- 摘要按“任务—方法—关键结果—验证/适用边界”组织。每个子问题写实质方法和已验证结果，不写“本文核心优势不是算法名称”“与官方模板写法一致”“结果另行保存”等评价或流程说明，不在摘要中自评排版和美观。
- 题面要求的对象、时刻、场景和附件输出必须完整呈现；若正文容量有限，正文给关键表并准确指向已经逐格验收的正式附件，不能只列代表样本却声称完成全题。
- 数值精度与输入精度、数值误差和官方输出要求相称。除求根或验收确需外，不用无依据的过多小数；正文、摘要、表格和附件采用一致的舍入口径。
- 参考文献标题只生成一次；无官方要求时不在参考文献前强制换页。尾页自然留白允许存在，但不得用分页命令制造连续稀疏正文页。
- 图表采用矢量格式优先，保证灰度打印、色盲和缩印后仍可辨认；同时使用线型、点型、纹理或直接标注表达关键类别。图内不重复论文 caption，不用彩虹色、3D 柱图或纯装饰图。
- 图表数量由论证需要决定，不设置每问/全文固定张数。每图附近写出它支持的结论即可，不为满足固定行数堆砌描述。表格保持可读字号，过宽时拆表或调整列设计，不靠整体缩成小字。
...
- 求解器达到时间/迭代/节点上限但保留 incumbent 时，摘要、正文、表题和附件说明都必须称“可行基准/可行近似方案”，同时报告终止类别与 gap；gap 未知或无穷不得省略成“已收敛”。未达到正式导出政策的模板只能作为带 `DRAFT_FEASIBLE_ONLY` 标识的审计草稿引用。
...
- 轮作、排班或多季土地计划首次给出结果时必须说明真实季序、历史边界、模式互斥、相邻禁配和滚动窗口如何逐项复核。混种/拆分资源要说明子资源状态身份；“地块上种过一点豆类”不得写成“全部土地完成轮作”。题面未量化的集中度、便利性和最小规模须标为条件阈值或审计局限，并给敏感性/整形建议。能用精确规划求解却改用启发式时，正文须解释原因并将“最优”降级到候选域；有 MIP gap 时同段报告。
- 题目要求提交结果模板时，附录须给出模板写回范围、逐格复核数量、显式小数位与 tie-breaking 规则、半单位边界测试和模板结构未改变的证据；正文只引用已通过回写验收且满足导出政策的附件，不得引用临时 CSV 或手工复制表。
