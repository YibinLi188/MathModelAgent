...
%    - CUMCM 常用 Windows 环境，因此本模板默认使用 ctex 的 windows 字体集；
%      不执行 Times New Roman/Menlo 等可选字体探测，避免缺失字体时停滞。
%    - macOS/Linux 使用者应在复制模板后显式改为 fontset=mac/fandol，并先做一次
%      最小编译验证；不要在正式论文首次编译时自动猜测平台。
...
% 中文宋体/黑体/楷体由 ctex 的 fontset=windows 设置，
